...
| 类型             | 格式               | 用途说明                                        |
| ---------------- | ------------------ | ----------------------------------------------- |
| JSON 数据        | `.json`            | 常用于热更新配置、商城、UI 数据、文本映射       |
| CSV / 表格       | `.csv`, `.xls(x)`  | 多语言文本、策划数据、关卡参数                  |
| 文本文件         | `.txt`, `.ini`     | 简单配置、对白、说明文字                        |
| XML 文件         | `.xml`             | 结构化数据，部分老项目使用                      |
| ScriptableObject | `.asset`           | 策划配置角色属性、技能、任务等，支持可视化编辑  |
...
- 开启压缩能减少包体积，但会增加加载时间
- 资源是否分包（AssetBundle/Addressables）影响更新和加载效率
...
- Canvas 是所有 UI 元素的父对象，控制 UI 渲染方式和分辨率适配。
- **Render Mode（渲染模式）**：
...
- 游戏运行时，场景或角色会处于不同的“状态”（如开始、暂停、游戏中、失败、胜利等）。
...
- **条件触发**：根据玩家操作或游戏事件，改变当前状态。

- **状态机设计**：用有限状态机（FSM）设计模式，明确定义状态和切换条件。

- 枚举控制的 FSM 框架，帮助初学者更好理解状态切换机制。
...
热更新是指在游戏发布后，不需要重新发布客户端应用即可更新游戏内容或逻辑的技术。Unity 中常见的热更新方式主要有以下几种：
...
- **XLua**：用于 Lua 脚本的热更新，动态运行和修改。
- **ILRuntime**：一个 .NET 运行时，可以动态加载并执行 C# 脚本，支持热更新。
